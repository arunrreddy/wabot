...
module.exports = {
    addContact: (db, jid) => {
        ...
        db.insert().into(contactItem).values([contactRow]).exec().then((rows) => {
        }).catch((e) => {
        });
    },
    getContacts: (db) => {
        var contactItem = db.getSchema().table('Contact');
        return db.select().from(contactItem).exec();
    },
    getContactByJid: (db, jid) => {
        var contactItem = db.getSchema().table('Contact');
        return db.select().from(contactItem).where(contactItem.jid.eq(jid)).exec();
    },
    getTwoContacts: (db, jid1, jid2) => {
        var contactItem = db.getSchema().table('Contact');
        return db.select().from(contactItem).where(lf.op.or(
            contactItem.jid.eq(jid1),
            contactItem.jid.eq(jid2)
        )).exec();
    },
    checkAdmin: (db, jid) => {
        var contactItem = db.getSchema().table('Contact');
    ...
    updateName: (db, jid, name) => {
        ...
        var contactItem = db.getSchema().table('Contact');
				db.select().from(contactItem).where(contactItem.jid.eq(jid)).exec().then((contact) => {
						var memos = contact[0].memos ? contact[0].memos : [];
						memos.push(memo_text);
						db.update(contactItem).
								set(contactItem.memos, memos).
								where(contactItem.jid.eq(jid)).exec();
				});
		},
		clearMemos: (db, jid) => {
        var contactItem = db.getSchema().table('Contact');
