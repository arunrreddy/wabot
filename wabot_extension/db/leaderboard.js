module.exports = {
	add_score: (db, group_jid, game, jid) => {
		...
		db
			...
			.where(leaderboardItem.group_jid.eq(group_jid))
			.exec()
			.then((group_board) => {
				if (group_board.length > 0) {
					// board exists
					var gameboard = group_board[0].games ? group_board[0].games : {};
					gameboard[game][jid] = gameboard[game][jid]
						? gameboard[game][jid] + 1
						: 1;
					db
						.update(leaderboardItem)
						.set(leaderboardItem.games, gameboard)
						.where(leaderboardItem.group_jid.eq(group_jid))
						.exec();
				} else {
					var gameboard = {};
