module.exports = {
	init: (port, log, db) => {
		...
		var base_url =
			"http://mobilews.365scores.com/Data/Games/?lang=10&AppType=2&uc=124&usc=124&tz=24&countries=&competitions=549,5447,7,5096,8,6346,5930,11,13,15,472,570,572,573,639&competitors=480,224,225,226,227,341,8346,331&FullCurrTime=true&onlyvideos=false&withExpanded=true&startdate=";
			...
			axios.get(game_url).then((response) => {
				...
				response.data.Games.forEach((game) => {
					games[game.ID] = {
						...
						home_b: game.Comps[0].SName
							? game.Comps[0].SName
							: game.Comps[0].Name,
						away: game.Comps[1].Name,
						away_b: game.Comps[1].SName
							? game.Comps[1].SName
							: game.Comps[1].Name,
						seq: 0,
						found_goal: false,
			...
			axios
				...
				.then((response) => {
					...
					var posts = _.filter(unfiltered, function(element) {
						return (
							element.data.author === "gemifra" ||
							element.data.author === "paicmhsc" ||
							element.data.author === "HerbalDreamin" ||
							element.data.author === "Meladroit1" ||
							element.data.author === "Adnell" ||
							element.data.author === "goonerlenny" ||
							element.data.author === "PradaBoyDave" ||
							element.data.author === "ennuihenry14" ||
							element.data.author === "TomasRoncero" ||
							element.data.author === "wazup564" ||
							element.data.author === "GoalsRR" ||
							element.data.author === "MmmBappe" ||
							element.data.author === "robert1811" ||
							element.data.author === "triza" ||
							element.data.author === "LordVelaryon" ||
							element.data.author === "TomasRoncero" ||
							element.data.author === "SweatiestPancake" ||
							element.data.author === "Badgersgopoo" ||
							element.data.author === "Ziqsu" ||
							element.data.author === "Fusir" ||
							element.data.author === "Meladroit40" ||
							element.data.author === "no_idea-to_nick" ||
							element.data.author === "dekani77"
						);
					});
										...
										if (post.data.url.includes("flixtc")) {
											...
											httpRequest.onload = function() {
												if (
													...
													var url = httpRequest.responseXML.getElementsByTagName(
														"source"
													)[0].src;
													var args = {
													...
													axios
														.post("https://api.cloudconvert.com/convert", args)
														.then((response) => {
															if (response.data.step === "finished") {
																goal_details.url =
																	"https:" + response.data.output.url;
																games[key].goals.push(goal_details);
																helpers.handleMultipleMedia(
																	api,
																	"https:" + response.data.output.url,
																	games[key].subscribed,
																	post.data.title,
																	null
																);
															}
											...
											httpRequest.open(
												"GET",
												"https://api.streamable.com/videos/" + video_id
											);
											httpRequest.onload = function() {
												if (this.status == 200) {
											...
											var video_id = result[2];
											var url =
												"https://clips.clippit.tv/" + video_id + "/720.mp4";
											var httpRequest = new XMLHttpRequest();
											...
											httpRequest.responseType = "document";
											httpRequest.send();
										} else if (post.data.url.includes("mixtape")) {
											...
											var text =
												...
												"\n" +
												post.data.title;
											games[key].subscribed.forEach((jids) => {
						...
						if (response.data.Notifications) {
							response.data.Notifications.forEach((notification) => {
								if (notification.EntID in games) {
									if (
										_.indexOf(games[notification.EntID].seen, notification.ID) <
										0
									) {
										...
										switch (notification.Type) {
											case 9: {
												var text =
													"🕐 *" +
													notification.Params[0].Value +
													"* " +
													games[notification.EntID].home +
													" vs " +
													games[notification.EntID].away;
											...
											case 10: {
												...
												var text =
													"⚽ *" +
													notification.Params[1].Value +
													"'* " +
													games[notification.EntID].home +
													" " +
													notification.Params[3].Value +
													"-" +
													notification.Params[4].Value +
													" " +
													games[notification.EntID].away;
												games[notification.EntID].events.push(text);
												games[notification.EntID].subscribed.forEach((jids) => {
											...
											case 53: {
												if (notification.Params[0].Value === "2") {
													"❌  *" +
														notification.Params[1].Value +
														"' Penalty Missed - " +
														notification.Params[2].Value +
		...
		return {
			subscribe: (message, index) => {
				...
				if (index >= 0 && index < keys.length) {
					...
					games[keys[index]].subscribed = lodash.uniq(
						games[keys[index]].subscribed
					);
					helpers.sendMessage(
						api,
						"Subscribed to match #" + (index + 1),
						message.object.from,
			...
			get_events: (message, index) => {
				...
				if (index >= 0 && index < keys.length) {
					var text =
						"*" +
						games[keys[index]].home +
						" vs. " +
						games[keys[index]].away +
						"*";
