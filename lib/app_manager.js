    ...
    init: (port, database, bugout) => {
        ...
        function start_app(message) {
            ...
            if (app) {
                ...
                if (_.findIndex(running_apps[jid], (element) => {
                        return element.name() === app.name();
                    }) == -1) {
                    bugout.log("[DEBUG]: App isn't running for " + jid + ", starting App");
                    var run = app.init(api, db, message.object.from, args, bugout, jid);
                    running_apps[jid] = running_apps[jid] || [];
                    running_apps[jid].push(run);
                    handle_events(run);
                } else {
                    helpers.sendMessage(api, "You are already running " + app.name(), message.object.from, message.object.id.id);
                }
            }
        }
        return {
