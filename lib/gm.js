            ...
            if (game) {
                if (game.isGroup() && message.isG) {
                    group.getGroupByJid(db, message.object.from).then((row) => {
                        var grp = row[0];
                        var players = removeMembers(grp.members.map((member) => {
                            return member.id;
                        }));
                        already_playing = already_playing.concat(players);
                        if (players.length) {
                            // Enough Players
                            var run = game.init(api, db, message.object.from, players, args, bugout);
                    ...
                    contact.getTwoContacts(db, jid, elements[1] + "@c.us").then((rows) => {
                        if (rows.length == 2) {
                            var allowed_players = removeMembers([jid, elements[1] + "@c.us"]);
