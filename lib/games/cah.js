...
module.exports = {
    ...
    init: (port, database, grp, members, args, bugout) => {
                ...
                if (scoreLimit()) {
                    ...
                    for (var jid in Players) {
                        if (Players[jid].Winner) {
                            text = text + '\n*' + Players[jid].Name + "* - " + Players[jid].Points + ' points (Winner)';
                        } else {
                            text = text + '\n*' + Players[jid].Name + "* - " + Players[jid].Points + ' points';
                        }
                    }
        ...
        function cycle() {
                    ...
                    if (result) {
                        ...
                        if (numofempty) {
                            ...
                            api.postMessage({
                                type: "send_text",
                                to: jid,
                                body: "Send your blank card inputs (separated with underscore if more than one)"
                            });
                        } else {
                            playedCards.push(played);
                            handler.deleteHandler(jid);
            ...
            var text = "*Round:* " + roundNum + "\n*Question:* " + questionText + "\n*Card Czar:* " + Players[czarJid].Name;
            api.postMessage({
                type: "send_text",
                to: group_jid,
                body: text
        ...
        function replaceCards() {
            ...
            for (var jid in Players) {
                ...
                api.postMessage({
                    type: "send_text",
                    to: jid,
                    body: "Select the cards you want to replace separated by spaces\n -Send 0 to replace none\n -Send \"all\" to replace all cards"
                });
            }
        }
        ...
        var text = "Cards Against Humanity starting\n *Score limit:* " + score_limit + "\n -Send 1 to join\n -Send 2 to decline";
        api.postMessage({
            type: "send_text",
            to: group_jid,
            body: text
