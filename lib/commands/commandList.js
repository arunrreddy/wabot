...
var scrr = require('../soccer.js');
var config = require('../config.js');
var lodash = require('lodash');
module.exports = {
    init: (database, port, gm, app_manager, bugout) => {
        var db = database;
        var api = port;
        var gm = gm;
        var am = app_manager;
        var soccer = scrr.init(api, bugout, db);
        ...
        var command_list = {
            ...
            "!ping": {
                action: function(message) {
                    helpers.sendMessage(api, "Pong", message.object.from, message.object.id.id);
                },
                description: "Ping",
                admin: false
            },
                ...
                action: (message) => {
                    ...
                    then((row) => {
                        ...
                        if (result) {
                            ...
                            helpers.sendMessage(api, "Only an admin can do that", message.object.from, message.object.id.id);
                        }
                    });
                },
            ...
            "!changenick": {
                action: (message) => {
                    ...
                    helpers.sendMessage(api, "Nick changed", message.object.from, message.object.id.id);
                },
            ...
            "!nick": {
                action: (message) => {
                    var jid = helpers.getUserJidFromMessage(message);
                    contact.getContactByJid(db, jid).
                    then((row) => {
                        helpers.sendMessage(api, row[0].nick, message.object.from, message.object.id.id);
                    });
                },
                    ...
                    if (jid === config.admin_jid) {
                        ...
                        then((row) => {
                            ...
                            if (result) {
                                ...
                                helpers.sendMessage(api, "Admin added", message.object.from, message.object.id.id);
                            } else {
                                text = "You are not an admin 🌚";
                                helpers.sendMessage(api, text, message.object.from, message.object.id.id);
                            }
                        ...
                        helpers.sendMessage(api, "Only bot owner can do that", message.object.from, message.object.id.id);
                    }
                },
                ...
                action: (message) => {
                    ...
                    then((row) => {
                        ...
                        if (result) {
                            ...
                            helpers.sendMessage(api, "Name changed", message.object.from, message.object.id.id);
                        } else {
                            text = "You are not an admin 🌚";
                            helpers.sendMessage(api, text, message.object.from, message.object.id.id);
                        }
            ...
            "!name": {
                action: (message) => {
                    ...
                    contact.getContactByJid(db, jid).
                    then((row) => {
                        helpers.sendMessage(api, row[0].name, message.object.from, message.object.id.id);
                    });
                },
            ...
            "!group": {
                action: (message) => {
                    ...
                    var number = 0;
                    if (message.isG) {
                        group.getGroupByJid(db, message.object.from).
                        then((row) => {
                            var grp = row[0];
                            ...
                            grp.members.forEach((member) => {
                                contact.getContactByJid(db, member.id).then((row) => {
                                    counter++;
                                    text = text + "\n *" + row[0].nick + "*: +" + row[0].jid.replace('@c.us', '');
                                    helpers.waitForSend(api, text, number, counter, message.object.from);
                                });
                            });
                        });
                    }
            ...
            "!games": {
                action: (message) => {
                    helpers.sendMessage(api, gm.listGames(), message.object.from, message.object.id.id);
                },
            ...
            "!gstatus": {
                action: (message) => {
                    helpers.sendMessage(api, gm.status(), message.object.from, message.object.id.id);
                },
            ...
            "!ghelp": {
                action: (message) => {
                    helpers.sendMessage(api, gm.help(), message.object.from, message.object.id.id);
                },
            ...
            "!leaderboard": {
              action: (message) => {
                ...
                if (game) {
                  contact.getContacts(db).
                  then((rows) => {
                    var contacts = lodash.zipObject(lodash.map(rows, 'jid'), lodash.map(rows, 'nick'));
                      ...
                      if (board) {
                       ...
                       if (game_board) {
                        ...
                        for (var i = 0; i < keys.length; i++) {
                          text = text + "\n" + (i+1) + ". *" + contacts[keys[i][0]] + "* - " + keys[i][1];
                        }
                        helpers.sendMessage(api, text, message.object.from, message.object.id.id);
                       } else {
                        helpers.sendMessage(api, "Leaderboard Empty", message.object.from, message.object.id.id);
                       }; 
                      } else {
                        helpers.sendMessage(api, "Leaderboard Empty", message.object.from, message.object.id.id);
                      };
            ...
            "!soccer": {
                ...
                description: "Subscribe to soccer games for goal highlights, use !soccer to list games",
                admin: false
            },
			"!goals": {
                action: (message) => {
                ...
                description: "Get Goal highlights for a game, use !soccer to list games",
                admin: false
			},
			"!events": {
                action: (message) => {
            ...
            "!request": {
                action: (message) => {
                    ...
                    if (request.length === 0) {
                        helpers.sendMessage(api, "Please enter a request", message.object.from, message.object.id.id);
                    } else {
                        request.addRequest(db, jid, text).
                        then((row) => {
                            var message_text = "[REQUEST] #" + row[0].id + "\n" + row[0].text;
                            helpers.sendMessage(api, message_text, config.admin_jid);
                            helpers.sendMessage(api, "Request sent", message.object.from, message.object.id.id);
                        });
                    }
                },
                description: "Send a request to the bot admin, he will contact you to address it.",
                admin: false
            },
            "!rstart": {
                action: (message) => {
                    ...
                    if (jid === config.admin_jid) {
                        if (request_id === 0) {
                            helpers.sendMessage(api, "Please enter a request id", jid, message.object.id.id);
                        } else {
            ...
            "!smmry": {
                action: (message) => {
                    ...
                    if (text.length > 0) {
                        var api_url = "http://api.smmry.com?SM_API_KEY=" + smmry_key + "&SM_WITH_BREAK" + "&SM_URL=" + text;
                        axios({
                            method: "post",
                            url: api_url
                        }).then((response) => {
                            if (response.data.sm_api_error) {
                                ...
                                helpers.sendMessage(api, message_text, message.object.from, message.object.id.id);
                            } else {
            ...
            "!flip": {
                action: (message) => {
                    ...
                    if (x) {
                        helpers.sendMessage(api, "Heads", message.object.from, message.object.id.id);
                    } else {
                        helpers.sendMessage(api, "Tails", message.object.from, message.object.id.id);
                    }
                },
            ...
            "!memos": {
                action: (message) => {
                    ...
                    then((rows) => {
                        ...
                        if (memos.length == 0) {
                            helpers.sendMessage(api, "You have no memos stored \n\n Store new memos by adding #memo to any message you send to the bot", jid);
                        } else {
                            var text = "```Memos:```\n";
            ...
            "!pick": {
                action: (message) => {
                    ...
                    helpers.zomato(api, message.object.from, message.object.id.id, city, category);
                }
            },
            "!apps": {
                action: (message) => {
                    helpers.sendMessage(api, am.list_apps(), message.object.from, message.object.id.id);
                },
